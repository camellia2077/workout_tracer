...
auto DatabaseManager::QueryAllLogs(sqlite3* sqlite_db)
  ...
  if (sqlite3_prepare_v2(sqlite_db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    std::cerr << "Failed to prepare query statement: " << sqlite3_errmsg(sqlite_db)
              << std::endl;
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    if (!temp_entries.contains(log_id)) {
      LogEntry entry;
      entry.date_ = reinterpret_cast<const char*>(sqlite3_column_text(stmt, kColDate));
      entry.exercise_name_ =
          reinterpret_cast<const char*>(sqlite3_column_text(stmt, kColExerciseName));
      ...
      const unsigned char* project_note_text = sqlite3_column_text(stmt, kColProjectNote);
      entry.project_note_ =
          (project_note_text != nullptr) ? reinterpret_cast<const char*>(project_note_text)
                            : "";
    ...
    detail.unit_ = (unit_text != nullptr) ? reinterpret_cast<const char*>(unit_text) : "kg";

    detail.elastic_band_weight_ = sqlite3_column_double(stmt, kColElasticBandWeight);
    const unsigned char* set_note_text = sqlite3_column_text(stmt, kColSetNote);
    detail.note_ =
        (set_note_text != nullptr) ? reinterpret_cast<const char*>(set_note_text) : "";

    temp_entries[log_id].sets_.push_back(detail);
  }

  if (sqlite3_prepare_v2(sqlite_db, "SELECT id, cycle_id FROM training_logs", -1,
  ...
  for (auto& [cycle_id, cycle_data] : data_by_cycle) {
    ...
    if (sqlite3_prepare_v2(sqlite_db, stats_sql, -1, &stmt, nullptr) == SQLITE_OK) {
      sqlite3_bind_text(stmt, 1, cycle_id.c_str(), -1, SQLITE_STATIC);
      if (sqlite3_step(stmt) == SQLITE_ROW) {
        cycle_data.total_volume_ = sqlite3_column_double(stmt, 0);
        cycle_data.average_intensity_ = sqlite3_column_double(stmt, 1);
        cycle_data.session_count_ = sqlite3_column_int(stmt, 2);
        cycle_data.vol_power_ = sqlite3_column_double(stmt, 3);
        cycle_data.vol_hypertrophy_ = sqlite3_column_double(stmt, 4);
        cycle_data.vol_endurance_ = sqlite3_column_double(stmt, 5);
      }
      sqlite3_finalize(stmt);
    }
  }
...
auto DatabaseManager::QueryPRSummary(sqlite3* sqlite_db) -> std::vector<PRRecord> {
  const char* sql = 
  ...
  if (sqlite3_prepare_v2(sqlite_db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      PRRecord record;
      record.exercise_name_ = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
      record.max_weight_ = sqlite3_column_double(stmt, 1);
      record.reps_ = sqlite3_column_int(stmt, 2);
      record.date_ = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));
      
      // Basic 1RM calculation logic (epley & brzycki)
      if (record.reps_ > 1) {
        record.estimated_1rm_epley_ = record.max_weight_ * (1.0 + record.reps_ / 30.0);
        record.estimated_1rm_brzycki_ = record.max_weight_ * 36.0 / (37.0 - record.reps_);
      } else {
