...
auto DatabaseHandler::Handle(const AppConfig& config) -> AppExitCode {
  ...
  if (config.action_ == ActionType::QueryPR ||
    ...
    if (config.action_ == ActionType::QueryVolume) {
      ...
      double avg_daily_vol = TrainingMetricsService::AverageDailyVolume(
          stats.total_volume, stats.total_days);
      double frequency = TrainingMetricsService::SessionsPerWeek(
          stats.session_count, stats.total_days);
      double density = TrainingMetricsService::SetsPerDay(stats.total_sets,
                                                          stats.total_days);
